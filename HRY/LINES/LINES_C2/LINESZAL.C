	 ...
	 for (j=0 ; j<random(130)+100;++j)	/* cyklus jedn‚ barvy */
		{
		 ...
		 line (Bufx1[i],Bufy1[i],Bufx2[i],Bufy2[i]); /* vymaz n¡ ‡ar */
                 line (Bufx1[i],MaxY-10-Bufy1[i],Bufx2[i],MaxY-10-Bufy2[i]);
		 if ((X1+dx1) < 0      ) dx1= random(4)+1; /* kontrola p©esahu */
		 if ((X1+dx1) > MaxX   ) dx1=-random(4)-1;
		 if ((Y1+dy1) < 0      ) dy1= random(4)+1;
		 if ((Y1+dy1) > MaxY-10) dy1=-random(4)-1;
                 if ((X2+dx2) < 0      ) dx2= random(4)+1;
		 if ((X2+dx2) > MaxX   ) dx2=-random(4)-1;
		 if ((Y2+dy2) < 0      ) dy2= random(4)+1;
		 if ((Y2+dy2) > MaxY-10) dy2=-random(4)-1;
		 ...
		 line (X1,MaxY-10-Y1,X2,MaxY-10-Y2);	/* zrcadlovˆ obr cen  ‡ ra */
		 Bufx1[i]=X1;
		 Bufx2[i]=X2;
		 Bufy1[i]=Y1;
		 Bufy2[i]=Y2;
