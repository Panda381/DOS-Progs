...
public:
	...
	{
		m_Name = name;
		m_FileName = path + _T("\\") + name + _T("." GRAPHEXT);
		m_ImgFileName = path + _T("\\") + name + _T("." PREVEXT);
		m_VarPath = path + _T("\\") + name;
	}
	...
	// variation path
	inline CText VarPath() const { return m_VarPath; }
...
public:
	...
	// entries list
	CGraphEntry* List() { return &m_List[0]; }
	...
	inline CGraphEntry& operator[] (int inx) { return m_List[inx]; }
	inline const CGraphEntry& operator[] (int inx) const { return m_List[inx]; }
	...
	// variation image filename with path
	...
	// variation path (empty = none)
	CText VarPath(int inx) { return this->IsValid(inx) ? m_List[inx].VarPath() : EmptyText; }
	CText VarPath() { return this->VarPath(m_SelInx); }
	...
	inline CGraphEntry* Selected() { return this->IsValid(m_SelInx) ? &m_List[m_SelInx] : NULL; }
